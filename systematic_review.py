#!/usr/bin/env python3
"""
Systematic review of all Mark Dickeson results
Extracts full text using SDK, reviews each article for relevance
"""
import sys
...
def review_mark_dickeson():
    client = TroveClient.from_env()
    
    search_term = '"Mark Dickeson"'
    print(f"\n{'='*80}")
    print(f"SYSTEMATIC REVIEW: {search_term}")
    print(f"{'='*80}\n")
            ...
            for category in page.categories:
                if 'records' in category and 'article' in category['records']:
                    ...
                    for article in articles:
                        ...
                        if text and text != "[Text not available in article metadata]":
                            print(f"    Text: {text[:200]}...")
                        print()

if __name__ == "__main__":
    articles = review_mark_dickeson()
    
    # Save results to JSON for analysis
    output_file = Path(__file__).parent / "mark_dickeson_results.json"
