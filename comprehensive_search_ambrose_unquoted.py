#!/usr/bin/env python3
"""
Systematic search for Ambrose using unquoted term to catch OCR variants
"""
import sys
...
def search_ambrose_unquoted():
    client = TroveClient.from_env()
    
    # Unquoted search gets more results but needs filtering
    search_term = 'Ambrose Dickeson'
    print(f"\n{'='*80}")
    print(f"AMBROSE DICKESON SEARCH (unquoted to catch OCR variants)")
                    ...
                    for article in articles:
                        ...
                        if date and len(date) >= 4:
                            ...
                            if year >= 1890:
                                ...
                                # Save results
                                output_file = Path(__file__).parent / "ambrose_dickeson_unquoted_results.json"
